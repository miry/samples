...
class Moon
  ...
  def self.parse(input : String)
    m = /.*x=(?<x>-?\d+),\s*y=(?<y>-?\d+),\s*z=(?<z>-?\d+)/.match(input)
    m = m.not_nil!
    x, y, z = m["x"].to_i64, m["y"].to_i64, m["z"].to_i64
    Moon.new(x, y, z)
  end
  ...
  def initialize(@x : Int64, @y : Int64, @z : Int64)
    @vel = [0_i64, 0_i64, 0_i64] of Int64
  end
  ...
  def step
    @x, @y, @z = @x + @vel[0], @y + @vel[1], @z + @vel[2]
  end

  def velocity_diff(x, other_x)
    return 1_i64 if x < other_x
    return -1_i64 if x > other_x
    0_i64
  end
