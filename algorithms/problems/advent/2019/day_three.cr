...
alias Coord = Tuple(Int32, Int32, Int32)
...
class Wire
  ...
  def add(commands : Array(String))
    ...
    commands.each do |command|
      x, y = step(command)
      new_position = {position[0] + x, position[1] + y, position[2] + x.abs + y.abs}
      if x == 0
  ...
  def intersection(other : Wire)
    result = self.class.intersection_vertical(self, other)
    result += self.class.intersection_vertical(other, self)
    result
  end

  def self.intersection_vertical(wire, other)
    result = Set(Coord).new
    wire.verticals.each do |vline|
      x = vline[0][0]
      ...
      other.horizonatls.each do |hline|
        ...
        if x > x1 && x < x2 && y > y1 && y < y2
          result << {x, y, vline[0][2] + (x - hline[0][0]).abs + hline[0][2] + (y - vline[0][1]).abs}
        end
      end
    end
    result
...
class Grid
  ...
  def closest
    ...
    min = coord[2]
    sets.each do |coord|
      min = coord[2] if min > coord[2] && coord[2] > 0
    end
    min
  end
end
