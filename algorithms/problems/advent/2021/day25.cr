...
class SeaCucumber
  ...
  def step
    new_map = Array(Array(Char)).new(@height)
    @height.times do |i|
      new_map << Array(Char).new(@width, '.')
    end
    puts "old:"
  ...
  def self.parse(input) : self
    plan = Array(Array(Char)).new
    ...
    return SeaCucumber.new(plan)
  end
end

def problem25(records : Array(String))
  cucumbers = SeaCucumber.parse(records)
...
def problem25_part_two(records : Array(Int64))
