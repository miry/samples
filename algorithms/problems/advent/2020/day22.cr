...
class SpaceCardsGame
  property decks : Array(SpaceCardsDeck)
  ...
  def play
    while @decks.all?(&.has_cards?)
      round
    end
    @winner = @decks[0].has_cards? ? @decks[0] : @decks[1]
  end

  def play2
    history1 = Hash(String, Int32).new
    history2 = Hash(String, Int32).new
    ...
    while @decks.all?(&.has_cards?)
      # puts "-- Round #{round_number} (Game #{@id}) --"
      # @decks.each do |deck|
      #   puts "Player #{deck.id}'s deck: #{deck.cards.join(',')}"
      # end

      if history1.has_key?(@decks[0].str) || history2.has_key?(@decks[1].str)
        @winner = @decks[0]
        return @winner
      end

      history1[@decks[0].str] = round_number
      history2[@decks[1].str] = round_number
      ...
      if (cards[0] <= @decks[0].cards.size) &&
        ...
        # puts "...anyway, back to game #{@id}."
        # puts "Player #{winner.id} wins round #{round_number}  of game #{@id}!"
        if winner.id == 1
          @decks[0].add cards
        else
          @decks[1].add cards.reverse
        end
      else
        # Note that the winner's card might be the lower-valued of the two cards if they won the round due to winning a sub-game.
        if cards[0] > cards[1]
          # puts "Player 1 wins round #{round_number}  of game #{@id}!"
          @decks[0].add cards
        else
          # puts "Player 2 wins round #{round_number}  of game #{@id}!"
          @decks[1].add cards.reverse
        end
      end
    ...
    # puts "Player #{@decks[0].id}'s deck: #{@decks[0].cards.join(',')}"
    # puts "Player #{@decks[1].id}'s deck: #{@decks[1].cards.join(',')}"
    # puts "The winner of game #{@id} is player #{@winner.id}!"
    @winner
  end
  ...
  def self.parse(decks)
    player_decks = [] of SpaceCardsDeck
    player_deck = [] of Int32
    player_id = 0
    decks.each do |line|
      if line == ""
        player_decks << SpaceCardsDeck.new(player_deck, player_id)
        player_deck = [] of Int32
        next
      end
      ...
      player_deck << line.to_i32
    end

    player_decks << SpaceCardsDeck.new(player_deck, player_id) if !player_deck.empty?

    self.new(player_decks)
  end
end
