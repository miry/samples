...
class SimpleRegexp
  ...
  def initialize(rules : Array(String))
    ...
    compress
    # @rules.keys.sort.each do |rule_id|
    #   rule = @rules[rule_id]
    #   puts "#{rule_id} : #{rule}"
    # end
  end
  ...
  def compress
    ...
    @rules.each do |rule_id, conditions|
      ...
      @rules[rule_id] = new_conditions
    end
  end

  def match_rule(str) : Bool
    rs = match(str.chars, 0, "")
    !rs.empty? && rs[0] == str.size
  end

  def match(str : Array(Char), rule_id = 0, ident = "") : Array(Int32)
    results = [] of Int32

    rule = @rules[rule_id]

    if rule.is_a?(String)
      results << 1 if str == rule.char_at(0)
    else
      rule.each do |sub_rule|
        rs = match(str, sub_rule, ident + "     ")
        results += rs if !rs.empty?
      end
    end
  ...
  def match(str : Array(Char), rule : Array(Int32 | String), ident = "") : Array(Int32)
    return [0] if rule.empty?
    return [] of Int32 if str.size < rule.size

    rs = [] of Int32
    rule_shift = 1
    first_rule = rule[0]
    if first_rule.is_a?(String)
      return [] of Int32 if str[0] != first_rule.char_at(0)
      rs = [1]
    else
      rs = match(str, first_rule, ident + " ")
      return [] of Int32 if rs.empty?
    end
  ...
  def self.validate(messages, version = 1)
    ...
    regexp = SimpleRegexp.new(rules)

    result = 0
    i += 1
    msg = messages[i]
    while msg != ""
      result += 1 if regexp.match_rule(msg)
      i += 1
      break if i == messages.size
      msg = messages[i]
    end
